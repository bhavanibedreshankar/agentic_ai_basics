...
- **Fan-out in one turn** — `run_turn` walks every `tool_use` block in a single API response (a model can request several spawns at once) and answers all of them in one combined `tool_result` message before continuing the loop, the same multi-block handling used in `../../07_Multi_Agent_Systems/orchestrator/orchestrator.py`'s `run_turn`.
...
python3 16_Dynamic_Agent_Spawning/dynamic_agent_spawning/dynamic_agent_spawning.py
```
...
- `../../07_Multi_Agent_Systems/orchestrator/README.md` — the fixed-roster version of delegation this template deliberately removes the "fixed" part of
- `../../07_Multi_Agent_Systems/worker_agent/README.md` — a sub-agent with its own internal tool-calling loop, pre-instantiated rather than assembled at call time
- `../../15_Self_Evolving_Agents/self_evolving_agents/README.md` — another template where a cap on a model-controlled quantity (rules learned, sub-agents spawned) is enforced entirely in code
