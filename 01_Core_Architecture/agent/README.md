...
- **`MAX_STEPS` as an unconditional backstop** — same idea as `../../02_Execution_Loops/max_iterations/max_iterations.py`: autonomy without a hard cap is dangerous, since a confused agent could otherwise loop on tool calls indefinitely.
...
python3 01_Core_Architecture/agent/agent.py
```
...
- `../../02_Execution_Loops/max_iterations/README.md` — the unconditional-cap pattern `MAX_STEPS` borrows
