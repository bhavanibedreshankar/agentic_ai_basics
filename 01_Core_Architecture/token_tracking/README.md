...
A chat agent (same shape as `../../02_Execution_Loops/agentic_loop/basic_agentic_loop.py`) that prints token usage and estimated cost after every turn, plus a running session summary when you type `exit`.
...
- **Cumulative tracking** — the `SessionUsage` class accumulates totals across every turn, since a multi-turn chat resends its full history on every call, so cost compounds as the conversation grows. This template doesn't trim history, so you can watch that growth happen — pair it with `../../05_Memory/memory_management/basic_agentic_memory.py`'s `trim_history` if you want to bound it.
...
- `../../05_Memory/memory_management/basic_agentic_memory.py` — bounding context growth (and therefore cost) with a sliding window
- `../llm_backbone/README.md` — comparing this same `usage` data across different models to see how the backbone affects cost
