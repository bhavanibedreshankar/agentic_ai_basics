...
- **Thread isolation** — a different `thread_id` starts from a blank slate, same guarantee [`../../17_LangChain/memory/README.md`](../../17_LangChain/memory/README.md) makes for `session_id`.
- This file is the concrete answer to `../../17_LangChain/memory/memory.py`'s HONESTY NOTE, which names checkpointing as the recommended replacement for `RunnableWithMessageHistory` — see that file's README for the "persists ONE thing vs. persists the ENTIRE state" distinction.
- Contrast with [`../../06_Task_and_State_Management/checkpointing/README.md`](../../06_Task_and_State_Management/checkpointing/README.md), which gets real cross-process durability today by writing its own JSON file by hand — `InMemorySaver` here doesn't survive a restart (see the module's HONESTY NOTE).
...
python3 18_LangGraph/persistence_and_checkpointing/persistence_and_checkpointing.py
```
...
- [`../../17_LangChain/memory/README.md`](../../17_LangChain/memory/README.md) — the deprecated mechanism this file replaces
