...
alone isn't new (../../17_LangChain/chains/chains.py's `RunnableBranch` and
../../17_LangChain/langgraph_workflows/langgraph_workflows.py already pick between two fixed
...
../../02_Execution_Loops/max_iterations/max_iterations.py caps a loop at a
fixed number of turns so it can't run forever, and
../../02_Execution_Loops/interrupts_breakpoints/interrupts_breakpoints.py
...
def write_draft(state: PitchState, llm: BaseChatModel) -> dict:
    if state["feedback"]:
        ...
        # step's output" idea ../../17_LangChain/chains/chains.py's `|`
...
def check_draft(state: PitchState) -> dict:
    ...
    # ../../08_Agent_Frameworks_and_Patterns/prompt_chaining/basic_prompt_chaining.py's validate_outline — cheap,
