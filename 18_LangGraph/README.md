...
The graph-orchestration library `17_LangChain/agents_and_tools/agents_and_tools.py`'s `create_agent` is built on, explored directly. [`../17_LangChain/langgraph_workflows/`](../17_LangChain/langgraph_workflows/README.md) is the entry point into this topic — a single 3-node branching graph shown from inside the LangChain topic; everything here goes deeper into the primitives that file uses without dwelling on: state-merge reducers, cycles, checkpointed persistence, first-class pause/resume, incremental observation, and subgraph composition.
