...
- **`START` / `END`** — the singleton markers every graph begins and ends at; `add_edge(START, "validate_order")` is what `../../17_LangChain/langgraph_workflows/langgraph_workflows.py`'s `set_entry_point()` wraps.
...
- Contrast with [`../../05_Memory/working_memory/README.md`](../../05_Memory/working_memory/README.md)'s scratchpad, where a plain Python dict a function mutates is what accumulates state — here the graph framework does the accumulating, driven by a type annotation.
...
python3 18_LangGraph/state_graph_basics/state_graph_basics.py
```
...
- [`../../17_LangChain/langgraph_workflows/README.md`](../../17_LangChain/langgraph_workflows/README.md) — the entry point for this topic, a simpler 3-node graph with no reducer
