...
- **Fixed control flow, model-generated content** — `run_chain` calls the three steps in a hardcoded sequence decided by your code, unlike `../tool_use/basic_agentic_tools.py`'s agentic loop, where the *model* decides what happens next and how many steps it takes. This makes it a "workflow", not an autonomous agent.
...
- `../tool_use/basic_agentic_tools.py` — the contrasting pattern where the model, not your code, decides the sequence of steps
