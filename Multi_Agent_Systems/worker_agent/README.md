...
- **`WorkerAgent`** — a class wrapping a system prompt, a tool set, and a tool executor into one `.run(task)` method. Internally it runs a complete tool-calling loop (structurally identical to `../../Core_Architecture/tool_use/basic_agentic_tools.py`'s `run_turn`) — however many turns that takes is invisible to the caller.
...
- `../../Core_Architecture/tool_use/README.md` — the single-agent tool-calling loop `WorkerAgent.run()` is built on
