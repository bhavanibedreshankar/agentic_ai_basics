...
- **Ephemeral** — nothing here is written to disk. This is the memory every chat template in the repo (starting with `../../agentic_loop/`) has been relying on; this template makes explicit what "ephemeral" actually means by never persisting anything at all.
...
- **`evict_oldest_pair`** — the simplest possible eviction policy: drop the oldest user/assistant pair, unconditionally, once the limit is exceeded. Deliberately dumber than `../../context_management/pruning.py` (which prunes selectively) or `../../context_management/summarization.py` (which compresses rather than discards) — this template exists to show the raw problem those two solve, not to solve it well itself.
...
- `../../context_management/README.md` — smarter eviction strategies (pruning, summarization) that avoid this template's total, unrecoverable loss
