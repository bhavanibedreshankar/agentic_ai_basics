...
- **`tool_search_tool_bm25_20251119`** — Anthropic's real, native tool-search server tool (no beta header, generally available), declared the same way as `../../03_Tools_and_Actions/web_search/web_search.py`'s `web_search`. BM25 takes natural-language search queries; the sibling `tool_search_tool_regex_20251119` variant takes Python regex patterns instead.
...
python3 08_Agent_Frameworks_and_Patterns/dynamic_tool_selection/dynamic_tool_selection.py
```
...
- `../../03_Tools_and_Actions/web_search/README.md` — another native Anthropic server tool declared for real, with the same "server-side call gets no `execute_tool`" pattern
- `../../16_Dynamic_Agent_Spawning/dynamic_agent_spawning/README.md` — selecting among existing tools at runtime vs. inventing a new agent persona at runtime
