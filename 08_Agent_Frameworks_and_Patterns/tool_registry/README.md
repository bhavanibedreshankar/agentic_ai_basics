...
A utility agent (calculator, word counter, text case converter, text reverser, clock) built around a `TOOL_REGISTRY` catalog, instead of the hand-maintained list + if/elif chain used in `../../01_Core_Architecture/tool_use/basic_agentic_tools.py`. Type `exit` to end the conversation.
...
python3 03_Tools_and_Actions/tool_registry/basic_tool_registry.py
```
...
- `../../01_Core_Architecture/tool_use/basic_agentic_tools.py` — the simpler hand-maintained approach this pattern replaces
