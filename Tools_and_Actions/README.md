...
| **Client-side (custom)** | Your code, and you define the schema too | Yes | `api_connectors_mcp/`'s `get_weather`, `browser_computer_use/`'s page tools |

Every custom tool built elsewhere in this repo (`../tool_use/`, `../tool_registry/`, `../Memory/external_memory/`, and so on) is client-side custom — this directory is where the other two categories show up for the first time. `api_connectors_mcp/` also touches a fourth option in its reference-only closing section: MCP, where a *server you don't control* executes the tool, over the network, with no `execute_tool()` on your side either.
