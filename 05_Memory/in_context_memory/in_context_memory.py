...
../../02_Execution_Loops/agentic_loop/ and beyond has been using implicitly; this template
makes its two defining properties impossible to miss:
  ...
  2. LIMITED BY TOKEN SIZE — every message ever sent in this conversation
     ...
     ../../02_Execution_Loops/agentic_loop/README.md). That's cheap at first and expensive
...
same real count_tokens endpoint from ../../01_Core_Architecture/token_tracking/ to measure
...
../../06_Task_and_State_Management/context_management/pruning.py's selective pruning or
../../06_Task_and_State_Management/context_management/summarization.py's summarize-then-replace. The
...
def count_tokens(messages: list[dict]) -> int:
    """The real count_tokens endpoint (see ../../01_Core_Architecture/token_tracking/) — not a
...
def evict_oldest_pair(messages: list[dict]) -> list[dict]:
    ...
    ../../06_Task_and_State_Management/context_management/'s techniques exist to avoid.
    """
