...
- **Ephemeral** — nothing here is written to disk. This is the memory every chat template in the repo (starting with `../../02_Execution_Loops/agentic_loop/`) has been relying on; this template makes explicit what "ephemeral" actually means by never persisting anything at all.
- **Limited by token size** — `count_tokens` (the real endpoint, same as `../../01_Core_Architecture/token_tracking/`) measures the conversation before every call against `MAX_CONTEXT_TOKENS`.
- **`evict_oldest_pair`** — the simplest possible eviction policy: drop the oldest user/assistant pair, unconditionally, once the limit is exceeded. Deliberately dumber than `../../06_Task_and_State_Management/context_management/pruning.py` (which prunes selectively) or `../../06_Task_and_State_Management/context_management/summarization.py` (which compresses rather than discards) — this template exists to show the raw problem those two solve, not to solve it well itself.
...
python3 05_Memory/in_context_memory/in_context_memory.py
```
...
- `../../06_Task_and_State_Management/context_management/README.md` — smarter eviction strategies (pruning, summarization) that avoid this template's total, unrecoverable loss
