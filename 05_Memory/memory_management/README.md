...
- **Short-term memory** — the `messages` list, same as `../../02_Execution_Loops/agentic_loop/basic_agentic_loop.py`. Exists only for the current run.
...
- **Sliding-window trimming** — `trim_history` bounds the in-session conversation to the most recent `MAX_HISTORY_TURNS` messages, so token cost and latency don't grow unbounded as a chat gets longer. (For a more surgical pruning technique, see `../../06_Task_and_State_Management/context_management/pruning.py`.)
...
python3 05_Memory/memory_management/basic_agentic_memory.py
```
...
- `../../06_Task_and_State_Management/context_management/` — deeper dives into pruning, summarization, and retrieval as distinct context management strategies
- `../../01_Core_Architecture/token_tracking/basic_token_tracking.py` — a closer look at measuring and reasoning about token usage
