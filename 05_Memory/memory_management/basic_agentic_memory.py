...
sessions. It demonstrates three distinct memory techniques:
  ...
  1. SHORT-TERM MEMORY — the `messages` list, same idea as
     ../../02_Execution_Loops/agentic_loop/basic_agentic_loop.py. Lives only for this run of the program; gone the
...
# long-term memory. (See ../../01_Core_Architecture/tool_use/basic_agentic_tools.py for a deeper walkthrough of
...
def run_turn(messages: list[dict], memories: list[str]) -> None:
    ...
    ../../01_Core_Architecture/tool_use/basic_agentic_tools.py's run_turn.
    """
    while True:
