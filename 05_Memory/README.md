...
```bash
python3 05_Memory/semantic_memory/semantic_memory.py
```
...
`../09_RAG_and_Knowledge/embedding/` is what `external_memory/`'s embedding mechanic is built on, applied there to a fixed read-only knowledge base instead of a writable store.
