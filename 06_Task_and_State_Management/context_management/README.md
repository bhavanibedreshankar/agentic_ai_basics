...
- **`prune_old_tool_results`** — selectively replaces stale `tool_result` blocks with a placeholder, in place. More surgical than `../../05_Memory/memory_management/basic_agentic_memory.py`'s `trim_history`, which drops whole old *turns* wholesale.
...
- **`summarize`** — a narrow, single-purpose LLM call (same idea as `../../08_Agent_Frameworks_and_Patterns/prompt_chaining/basic_prompt_chaining.py`'s steps) whose only job is compressing older history into a short paragraph.
...
- Contrast with `../../05_Memory/memory_management/basic_agentic_memory.py`: memory always injects saved facts into every system prompt; retrieval pulls in only what's relevant to the *current* question, on demand.
