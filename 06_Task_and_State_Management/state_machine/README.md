...
- **Contrast with `../../07_Multi_Agent_Systems/agent_handoff/agent_handoff.py`** — that template also tracks a current-state string (`current_agent_name`), but *any* transfer tool can fire from *any* agent; there's no table of which transitions are even legal. This template is what handoff would need to add real state-legality enforcement.
...
python3 06_Task_and_State_Management/state_machine/state_machine.py
```
...
- `../../07_Multi_Agent_Systems/agent_handoff/README.md` — an unenforced current-state pattern this template's transition table formalizes
