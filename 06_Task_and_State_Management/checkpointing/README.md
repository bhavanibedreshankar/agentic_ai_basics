...
- **Persisting progress, not just data** — contrast with `../../05_Memory/episodic_memory/` and `../../05_Memory/semantic_memory/`, which persist facts or past interactions while the task producing them is expected to finish normally. This template persists an *in-flight, unfinished* task's progress, specifically so an interrupted run can continue rather than restart.
...
python3 06_Task_and_State_Management/checkpointing/checkpointing.py
```
...
- `../../04_Planning_and_Reasoning/plan_and_execute/README.md` — the same step-by-step execution shape without durability
