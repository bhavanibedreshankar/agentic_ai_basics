...
- **`ask_tier`** — one tier's attempt, returned as structured output (`{"answer": ..., "confident": ...}`) rather than a string marker to search for in free text — same "never free text to parse" principle as `../../08_Agent_Frameworks_and_Patterns/router_agent/router_agent.py`'s `CLASSIFY_SCHEMA`, applied to a confidence flag instead of a category.
...
python3 12_Model_Routing/complexity_based_escalation/complexity_based_escalation.py
```
