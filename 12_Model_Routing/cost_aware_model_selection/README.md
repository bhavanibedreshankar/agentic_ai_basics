...
- **`SessionBudget.record`** — converts a response's real `usage` into an estimated dollar cost and accumulates it, the same idea as `../../13_Benchmarking/latency_cost_benchmarking/README.md`'s cost tracking, applied per-tier here.
...
python3 12_Model_Routing/cost_aware_model_selection/cost_aware_model_selection.py
```
...
- `../../01_Core_Architecture/token_tracking/README.md` — general per-turn cost tracking, without a tier-selection decision attached
