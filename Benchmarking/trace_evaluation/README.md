...
- **`TraceStep` / `run_agent(task)`** — captures every tool call, tool result, and final answer from a real tool-calling loop (same inner-loop shape as `../../Core_Architecture/tool_use/basic_agentic_tools.py`'s `run_turn`) into a plain ordered list, independent of the Claude SDK's own message format.
...
python3 Benchmarking/trace_evaluation/trace_evaluation.py
```
...
- `../../LangChain/callbacks_and_tracing/README.md` and `../../Safety_and_Control/audit_trail/README.md` — capturing a trace as an agent runs, the step this template assumes has already happened
- `../../Agent_Frameworks_and_Patterns/evaluator_agent/README.md` — the reusable single-call scoring primitive both `check_trace_rules`'s pass/fail philosophy and `judge_trace` borrow from
