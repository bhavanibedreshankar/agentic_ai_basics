...
- **A dynamically generated plan, not a hardcoded sequence** — contrast with `../../08_Agent_Frameworks_and_Patterns/prompt_chaining/basic_prompt_chaining.py`, which always runs the same three fixed steps (outline → draft → edit). Here, `generate_plan` produces a *different* plan depending on the task — a simple request might get 3 steps, a complex one more.
...
python3 04_Planning_and_Reasoning/plan_and_execute/plan_and_execute.py
```
...
- `../../08_Agent_Frameworks_and_Patterns/prompt_chaining/README.md` — the fixed-sequence alternative this template's dynamic plan generalizes
- `../../01_Core_Architecture/tool_use/README.md` — how to extend `execute_step` with its own tools per step
