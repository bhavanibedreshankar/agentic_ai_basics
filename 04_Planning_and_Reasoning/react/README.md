...
- **Contrast with `../../01_Core_Architecture/tool_use/basic_agentic_tools.py`** — structurally the same loop (call tools, feed results back), but ReAct specifically makes the reasoning between actions part of the visible transcript. That's what turns plain tool use into ReAct.
...
python3 04_Planning_and_Reasoning/react/react_agent.py
```
...
- `../../01_Core_Architecture/tool_use/README.md` — the underlying tool-calling loop this template adds explicit reasoning to
