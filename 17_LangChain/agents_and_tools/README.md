...
- Contrast with [`../../01_Core_Architecture/tool_use/basic_agentic_tools.py`](../../01_Core_Architecture/tool_use/README.md), which hand-writes the identical loop: a `while True`, a `stop_reason == "tool_use"` check, and manual dispatch by tool name.
...
python3 17_LangChain/agents_and_tools/agents_and_tools.py
```
...
- `MODEL`, `MAX_TOKENS`, `SYSTEM_PROMPT` — see [`../../01_Core_Architecture/basics/README.md`](../../01_Core_Architecture/basics/README.md)
- `TOOLS` — add more `@tool`-decorated functions to extend what the agent can do

### See also

- [`../../01_Core_Architecture/tool_use/README.md`](../../01_Core_Architecture/tool_use/README.md) — the same tool-calling loop, written by hand
- [`../../18_LangGraph/README.md`](../../18_LangGraph/README.md) — the graph layer `create_agent` is built on, explored directly
