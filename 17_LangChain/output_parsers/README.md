...
- **`llm.with_structured_output(Model)`** — binds the schema as a tool the model must call, so arguments arrive pre-validated the way [`../../01_Core_Architecture/tool_use/basic_agentic_tools.py`](../../01_Core_Architecture/tool_use/README.md)'s `tool_input` does; no format instructions needed in the prompt.
...
python3 17_LangChain/output_parsers/output_parsers.py
```
...
- [`../../01_Core_Architecture/tool_use/README.md`](../../01_Core_Architecture/tool_use/README.md) — the raw-SDK tool-schema enforcement `with_structured_output` builds on
