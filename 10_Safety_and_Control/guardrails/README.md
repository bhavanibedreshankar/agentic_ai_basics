...
- **Automatic rejection, not a human pause** — contrast with `../../02_Execution_Loops/interrupts_breakpoints/interrupts_breakpoints.py`, which *pauses* for a human decision when a condition fires. Guardrails here reject outright, no human involved, with an explanation the model can act on.
- **Content rules, not state legality** — contrast with `../../06_Task_and_State_Management/state_machine/state_machine.py`, which checks whether a *transition* is legal from the current state. `check_action_guardrail` checks whether a call's *content* (a refund amount) is within policy, independent of any state machine.
...
python3 10_Safety_and_Control/guardrails/guardrails.py
```
...
- `../../02_Execution_Loops/interrupts_breakpoints/README.md` — the human-pause alternative to this template's automatic rejection
