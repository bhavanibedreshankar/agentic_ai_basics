...
- **Written for humans, never read back by the agent** — contrast with `../../06_Task_and_State_Management/checkpointing/checkpointing.py` (persisted so the *same agent* can resume) and `../../05_Memory/episodic_memory/episodic_memory.py` (persisted so the agent can *recall* it later). Nothing in this file loads the audit log back into a prompt — `print_audit_log` is the only reader, and it's meant for a human.
...
python3 10_Safety_and_Control/audit_trail/audit_trail.py
```
...
- `../../06_Task_and_State_Management/checkpointing/README.md` — persistence the agent reads back to resume, contrasted with this template's human-only record
- `../../05_Memory/episodic_memory/README.md` — persistence the agent reads back to inform its own future behavior, also contrasted with this template's human-only record
