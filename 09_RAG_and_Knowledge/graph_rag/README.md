...
python3 09_RAG_and_Knowledge/graph_rag/graph_construction.py
```
...
- **`find_seed_nodes`** — the same keyword-overlap scoring idea as `../../06_Task_and_State_Management/context_management/retrieval.py`'s `search_notes`, just scored against a graph node's name/qualname/docstring instead of a flat document.
...
python3 09_RAG_and_Knowledge/graph_rag/graph_retrieval.py
```
...
- `../../06_Task_and_State_Management/context_management/retrieval.py` — the flat-text `search_notes` this template's seed-matching is modeled on
...
- **Fixed pipeline vs. tool-calling**, the same contrast already drawn between `../rag/basic_rag.py` and `../../06_Task_and_State_Management/context_management/retrieval.py`'s `search_notes` tool — applied here to a graph instead of flat documents.
...
pip install -r requirements.txt
export ANTHROPIC_API_KEY=your-key-here
python3 09_RAG_and_Knowledge/graph_rag/graph_rag_agent.py
```
...
- `../../01_Core_Architecture/tool_use/basic_agentic_tools.py` — the tool-calling loop shape (`TOOLS` + `execute_tool` + `run_turn`) this template follows
