...
- **`evaluate_output(task, output, rubric)`** — the same shape as `../../08_Agent_Frameworks_and_Patterns/evaluator_agent/evaluator_agent.py`'s `evaluate_output`: task/output/rubric are caller-supplied, scoring is structured output (`EVALUATION_SCHEMA`), and `passed` is a **code** decision (`score >= PASS_THRESHOLD`), never left to the model.
...
python3 13_Benchmarking/llm_judge_benchmarking/llm_judge_benchmarking.py
```
...
- `../../08_Agent_Frameworks_and_Patterns/evaluator_agent/README.md` — the single-call scoring primitive this template loops over a whole suite
