...
- **`run_benchmark(suite)`** / **`BenchmarkReport.pass_rate`** — the harness that runs an entire suite in one pass and reduces it to a single aggregate number, which is what distinguishes *benchmarking* from the single-shot scoring in `../../08_Agent_Frameworks_and_Patterns/evaluator_agent/evaluator_agent.py`.
...
- `../../08_Agent_Frameworks_and_Patterns/evaluator_agent/README.md` — a single-call LLM-judge utility, contrasted with this template's zero-API-call scoring
