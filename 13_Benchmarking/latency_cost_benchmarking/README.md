...
- **`estimate_cost(input_tokens, output_tokens)`** — the same pricing-to-cost conversion as `../../01_Core_Architecture/token_tracking/basic_token_tracking.py`'s `estimate_cost`, here applied per-run across a sweep instead of accumulated over one chat session.
...
python3 13_Benchmarking/latency_cost_benchmarking/latency_cost_benchmarking.py
```
...
- `MODEL`, `MAX_TOKENS` — see `../../01_Core_Architecture/basics/README.md`
...
- `../../01_Core_Architecture/token_tracking/README.md` — the same usage-to-cost conversion, accumulated per session instead of swept per effort level
