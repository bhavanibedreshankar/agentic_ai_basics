...
- **One gate inserted into the ordinary tool loop** — contrast with `../../01_Core_Architecture/tool_use/basic_agentic_tools.py`, which dispatches a requested tool immediately. Here, `run_turn` checks `REQUIRES_APPROVAL` before dispatch; everything else in the loop is identical.
...
python3 02_Execution_Loops/human_in_the_loop/human_in_the_loop.py
```
...
- `../../01_Core_Architecture/tool_use/README.md` — the underlying tool-calling loop this template adds one approval check to
