...
The software-testing discipline for an agent's code, as distinct from `../13_Benchmarking/`'s job of SCORING an agent's live output quality: unit-testing tools in isolation, integration-testing the orchestration loop against a stubbed LLM, red-teaming with an adversarial corpus, extracting shared fixtures/mocks so the suite stays fast, and layering all of it plus `../13_Benchmarking/`'s expensive templates into one cost-aware pipeline as the agent and its test suite both grow.
...
| 5 | [`test_suite_pyramid/`](test_suite_pyramid/README.md) | Tying it all together — layering fast (1-3) and slow (LLM-judge/live, from `../13_Benchmarking/`) tiers into one pipeline, gated by cost |
...
```bash
python3 14_Agent_Testing/tool_unit_testing/tool_unit_testing.py
pytest 14_Agent_Testing/tool_unit_testing/tool_unit_testing.py
```

Run every fast-tier file in this topic together:

```bash
pytest 14_Agent_Testing/tool_unit_testing/ 14_Agent_Testing/agent_integration_testing/ 14_Agent_Testing/adversarial_safety_testing/ 14_Agent_Testing/fixture_and_mock_management/
```
