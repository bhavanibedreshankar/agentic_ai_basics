...
| 8 | [`08_Agent_Frameworks_and_Patterns/`](08_Agent_Frameworks_and_Patterns/README.md) | Compositional patterns for structuring how an agent turns a request into a result: a tool registry, native tool search over a large tool catalog, fixed prompt chaining, a classify-then-route agent, and a reusable output evaluator (reflection loop lives in `04_Planning_and_Reasoning/`) | `basic_tool_registry.py`, `dynamic_tool_selection.py`, `basic_prompt_chaining.py`, `router_agent.py`, `evaluator_agent.py` |
