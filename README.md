...
| 20 | [`20_Graph_Neural_Networks/`](20_Graph_Neural_Networks/README.md) | Graph Neural Networks taught through RTL/hardware design on a toy adder netlist: representing a netlist as a graph, the core message-passing mechanic (with a genuine over-smoothing demo), a trained GCN classifying timing-critical gates, an inductive GraphSAGE that generalizes to an unseen netlist, a GAT that learns which fan-in signal matters, and a survey of real EDA use cases | `graph_representation_of_netlists.py`, `message_passing_basics.py`, `gcn_gate_classification.py`, `graphsage_inductive_netlist_embedding.py`, `gat_attention_critical_path.py`, `gnn_applications_eda.py` |
